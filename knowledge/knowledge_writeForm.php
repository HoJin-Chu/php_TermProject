<!DOCTYPE html>
<html>

<head>
	<meta charset="utf-8">
	<title>게시판</title>
	<!--부트스트랩 링크-->
    <link rel="stylesheet" href="//maxcdn.bootstrapcdn.com/bootstrap/4.0.0-beta.3/css/bootstrap.min.css">
	<style>
		body {
			padding: 5%;
		}
		table tr>td:nth-child(1) {
			text-align: center;
		}
		textarea {
			height: 200px;
		}
	</style>
</head>
<body class="container">
	<h1 class="text-center">write</h1>
	<br>
	<div class="row justify-content-center">
		<div class="col-md-6">
			<!--submit하면 title과 content에 작성된 내용을 write.php로 전송-->
			<form method="post" action="knowledge_write.php">
				<table border="2" class="table table-hover">
					<tr>
						<td>Title</td>
						<td>
							<input type="text" class="form-control" name="title" autofocus> </td>
					</tr>
					<tr>
						<td colspan="2">
							<textarea name="content" class="form-control" placeholder="comment"></textarea>
						</td>
					</tr>
				</table>
				<button type="submit" class="btn btn-default btn-block">작성</button>
			</form>
			<button onclick="location.href='knowledge_board.php'" class="btn btn-danger btn-block">작성취소</a>
		</div>
	</div>
</body>

</html>