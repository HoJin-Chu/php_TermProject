...
?>
	...
	<link rel="stylesheet" href="/TermProject3/css/knowledgeboard.css">
		...
		<script>
			function deleteOk(num){
				...
				if(result){
					location.href="community_delete.php?num="+num;
				}
			}
		</script>
	</head>
    ...
    <?php require_once $_SERVER['DOCUMENT_ROOT']."/TermProject3/header.php" ?>
		<div class="container">
			...
			<hr>
			<!-- 세션값을 불러와서 비교를 한다 -->
			<?php $writer = sessionValue("userId"); ?>

			<!--지금 로그인한 아이디와 작성자가 같으면 수정과 삭제 가능-->
			<?php if ($writer == $result["writer"]) {?>
			<input type="button" class="btn" onclick="location.href='community_modifyForm.php?num=<?=$num?>'" value="수정">
			<input type="button" class="btn" onclick="deleteOk(<?=$num?>)" value="삭제">
			<!--지금 로그인한 아이디와 작성자가 아니거나 로그인하지않았으면 수정과 삭제 불가능-->
			<?php } else {?>
			...
			<?php }?>
			<input type="button" class="btn" onclick="location.href='community.php'" value="목록보기">
			<!--  -->
		 ...
		 <hr>
			...
			<form action="community_comment.php?num=<?=$num?>" method="post">
			...
			$msg = $boardDao->selectImgComment($num);
			?>
